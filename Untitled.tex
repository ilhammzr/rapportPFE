...
\usepackage[top = 3cm, bottom = 3cm, right = 2cm, left = 4.5cm]{geometry}  % marges%
...
\usepackage{enumitem}
\usepackage{tcolorbox} % entourage des résumés
...
%---------------- MOOOOOOOOOOOMO 
    ...
    {0.5pc}
    {\vspace{0.5pc}\Huge#1}%
    \titlespacing{\chapter}{0pt}{13\baselineskip}{2\baselineskip}
    ...
    [\ifbool{addtoc}{\addcontentsline{toc}{chapter}{#1}}]%
     \titlespacing{name = \chapter, numberless}{0pt}{1\baselineskip}{2\baselineskip}
...
\textsc{Mémoire de Projet de Fin d’Étude \newline Présenté en vue d'obtenir
\newline Le diplôme d'Ingénieur d’État
\newline Spécialité : Génie Informatique
\newline Option : Qualité Logiciel
\newline N° : *******
}

}\\[0.4cm] % Major heading such as course name
...
\HRule \\[0.3cm]
{ \huge \bfseries \textbf{Conception et réalisation d'une Solution CRM pour la gestion du processus Commercial de Neoxia Maroc à base du CRM Salesforce}}\\[0.1cm] % Title of your document
\HRule \\[1.5cm]
...
\textit{À mon ange gardien et mon fidèle compagnon dans les moments les plus délicats de cette vie mystérieuse, merci Pour tes encouragements et ton soutien, pour être là à chaque fois que j'en ai besoin, pour tes conseils, tes sacrifices, tu es mon deuxième père. Je t'aime.}
 
\textbf{\\À ma chère Ikram,\\}
\textit{Celle qui illumine mon existence, merci pour ta disponibilité tu était toujours à mes cotés dans les bons moment ainsi que les mauvais, les mots ne suffisent guère pour exprimer l'attachement, l'amour et  l'affection que je porte pour toi. Je t'aime.}

\textbf{\\À ma belle Bouchra,\\}
\textit{Ma source de force et d'inspiration, tu étais toujours là pour me remonter le moral lorsque ma détermination flanchait, merci pour ta disponibilité et ton amabilité. Je t'aime.}

\textbf{\\À mon petit frère Bilal,\\}
...
\textbf{\\À toute ma grande famille, A tous mes proches et amis qui n'ont jamais cessé de me fournir de toute sorte d'aide}.
...
C'est dans cet optique que Neoxia a investi pour créer une solution CRM pour gérer ses propres processus commerciales.
 ...
 \paragraph{}
Nous avons eu pour mission la mise en place d'une application visant à aider Neoxia à améliorer leurs performances et leur position concurrentielle. L'enjeu était la configuration de la solution Salesforce CRM pour réaliser un processus qui automatise les forces de vente en facilitant considérablement la tâche, accélérant le processus de vente, le suivi des projets ainsi que les imputations, et finalement la gestion de la facturation et le recouvrement. 

 \paragraph{}
Pour ce faire, nous avons commencé par une analyse détaillé, puis une conception complète pour définir toutes les fonctionnalités que l'entreprise voulait mettre en place, on a passé ensuite à modéliser les besoins spécifiés et les processus à adopter, et par la suite une longue phase de réalisation et de validation continue.

\paragraph{}
L'application est modélisée à l'aide du BPM Bizagi, et développée par des outils propres à la plateforme Force.com : le langage APEX, le Framework Visualforce et SOQL qui gère l'interaction avec la base de données.

\newline
\newline
\newline
 \paragraph{}
\textbf{Mots clé}: CRM, \textsc{salesforce.com}, Apex,BPM, VisualForce, SOQL, Force.com,Cloud Computing.
...
\addcontentsline{toc}{chapter}{\textsc{Abstract}}
 ...
 \paragraph{} 
In this report, I will try to summarize the work that we've done in my graduation internship mission, within the consultant firm \textbf{Neoxia Morocco}, a privileged integrator partner of Salesforce, established in Casablanca.

 \paragraph{}
In order to increase its productivity and profit while reducing time and budgets, and to improve its performance and competitive position, Neoxia has put the client satisfaction as the center of attention. That's why Neoxia has invested a lot of time and effort to create a CRM solution to manage its own commercial process.

 \paragraph{}
Our mission was about creating an application with the purpose of helping Neoxia to attain their objectives and to have a product that responds to their expectations. The issue was mainly on the configuration of the Salesforce CRM solution to achieve a process that automates and increases sales by considerably facilitating the task, accelerating the process of sale, monitoring the different projects and postings, and finally, taking care of the billing management.

 \paragraph{}
We started our mission by conducting a full diagnosis then a detailed conception of the solution, we defined all the functionalities that they needed to put in place, afterwards, we modelized all the specific needs, while deciding at each time which processes to adopt, a long phase of realization took place and we continuously validated parts of the project till we had our final product.

\paragraph{}
We used BPM Bizagi to modelize the application, and specific tools of the Force.com platform were used in the development phase, like: Apex language, Visualforce Framework and SOQL.
\newline
\newline
\newline
 \paragraph{}
\textbf{key words}: CRM, \textsc{salesforce.com}, VisualForce,BPM, APEX, SOQL, Visualforce, Force.com,Cloud Computing.
...
\addcontentsline{toc}{chapter}{\textsc{Introduction générale}}
...
\paragraph{}
En fait dernièrement Neoxia a connu une croissance importante du  nombre des collaborateurs, des marchés gagnés ainsi que le nombre des profils et des technologies utilisées,  ce qui rend difficile d'obtenir une vue entière de l'entreprise, d’analyser le résultat du marketing,  ou de suivre les projet et le rendement des ventes.
 \paragraph{}
Dans un deuxième temps les consommateurs aujourd'hui habitués à avoir le choix et l'accès à des produits de qualité portent une attention grandissante à des aspects autrefois relégués au second plan. Lors de son processus d'achat, le consommateur recherche des aspects comme le plaisir, la sécurité, la considération et la personnalisation. 
\paragraph{}
Ces nouveaux critères obligent les entreprises à mettre en place les processus nécessaires à la rencontre de ces attentes. C'est ce qui explique l'implantation accélérée de solutions visant la Gestion des Relations Client ou la "Customer Relationship Management" (CRM). 
 ...
 \paragraph{}
La mise en place d'un CRM redonne de la visibilité et de la sérénité car il répond aux besoins actuels. Il structure, partage et distille l'information appropriée pour tous les services. Cette centralisation est facilitée par la réalisation de nombreuses actions marketing et par l'intégration de différents services comme l'accès à la boite mail et aux divers documents électroniques.
\paragraph{}
  Mais Un CRM comme tout autre outil reste limité, et ne peut pas répondre à tous les besoins de l'entreprise, sachant qu'il doit être personnalisé d'une façon globale pour pouvoir s'adapté à un contexte informatique, où la « la vente » des prestations et la plus compliquée.
C'est dans cette optique qu'intervient mon sujet de stage de fin d'étude, Nous avons eu pour mission la mise en place d’une application visant à aider Neoxia à améliorer leurs performances et leur position concurrentielle. 
L'enjeu n'arrête pas à la configuration de la solution Salesforce CRM pour réaliser un processus qui automatise les forces de vente, mais aussi le développement de nouvelles fonctionnalités dans la plateforme, et l'amélioration des processus standard ainsi que la personnalisation de l'existant pour qu'il conforme au métier de l'entreprise cela en facilitant considérablement la tâche, et  accélérant le processus de vente, le suivi des projets ainsi que les imputations, et finalement la gestion de la facturation et le recouvrement


 \paragraph{}
Le présent rapport décrit l'ensemble des étapes de réalisation de notre mission au sein de l'organisme Neoxia. nous avons choisi de le présenter sous 5 chapitres :

\begin{itemize}
\item \textbf{Le premier chapitre} présente le contexte général du projet en présentant l'organisme d'accueil et en définissant les objectifs de notre mission ainsi que la conduite que nous avons suivi pour mener à bien notre projet.

\item \textbf{Le deuxième chapitre} a comme but d'expliquer la théorie du Cloud Computing, de la gestion des relations clients et la philosophie de Salesforce.

\item \textbf{Le troisième chapitre} est consacré à la branche fonctionnelle des processus adopté lors de la phase de conception, qui a permis un repérage des besoins fonctionnels et opérationnels du système. Il décrit la conception des fonctionnalités de la solution.

\item \textbf{Le quatrième chapitre} est consacré à la branche technique de l'application réalisée. Il expose les concepts techniques clés du projet et l'ensemble des technologies utilisées.

\item \textbf{Le dernier chapitre} est consacré à la mise en \oe uvre du projet en décrivant toutes les étapes par lesquelles nous sommes passés pour donner naissance à notre application depuis la configuration des objets personnalisés à la génération des rapports et tableaux de bord.

\end{itemize}

\paragraph{}
Le rapport s'achève par une conclusion résumant les résultats du développement de la solution tout en proposant des perspectives susceptibles d'améliorer et de compléter le projet en question.
\end{flushleft}
...
\chapter{\textsc{Contexte général de Neoxia Sales}}
%--- resumé 

\begin{tcolorbox}
Dans ce chapitre, nous introduirons le contexte général du projet. Nous présenterons d'abord l'organisme d'accueil. Ensuite, nous présenterons le contexte général du projet ainsi que ses principaux objectifs, une présentation de la conduite de ce projet en termes de gestion et de planification.
Ce chapitre se terminera par 3 axes sur la théorie général du projet : CRM , Cloud Computing et la philosophie de Salesforse
\end{tcolorbox}

%----- dirli les résumé f chi cadre wla chi l3ba ila kat3ref 

\newpage

\section{Organisme d'accueil: NEOXIA MAROC}

\paragraph{}
NEOXIA est un cabinet d'architecture et de gouvernance du SI. Depuis, 2000 à paris puis en 2008 à Casablanca, les Architectes NEOXIA accompagnent les grandes entreprises et administrations dans l'évolution de leur Système d'informations. En plaçant le SI comme un levier réel et important de création de valeur, NEOXIA permet à ses clients une adoption réfléchie, progressive et optimale de l'outil informatique. 
...
\caption{Répartition l'effectif par profil}
\end{figure}
\item{\textbf{Chiffre d'affaire}}
...
\caption{Chiffre d'affaire du Group Neoxia en Million d'Euros.}
\end{figure}
...
\caption{Chiffre d'affaire NEOXIA Maroc en Million DH.}
\end{figure}
...
\item  Capitaliser : Amélioration, Industrialisation, Réutilisation, Méthodes et bonnes pratiques
\end{itemize}
\begin{figure}[H]
\centering
\includegraphics[scale=0.42]{dec.png}
...
\paragraph{}
Neoxia comme un cabinet de conseil qui vient d'être installé sur le terrain marocain, cherche à gangner les bons marché ainsi de garder et fédiliserles bons clients.
 \paragraph{}
Durant ses 4 ans d'expérience, Neoxia remarque que son volume dans le marché marocain augmente au niveau du nombre des clients, les marchés gagnés, les actions commerciales et marketing, le nombres des technologies utilisée ainsi que la multitude des profils des Neoxiens, avec cette augmentation, le suivi des activités de l'entreprise devient difficile, à voir  le traitement de tous ce qui concerne l'identification des clients et surtout les besoins et attentes fluctuants.
En plus le grand nombre de projet sur lesquels on travaille simultanément diminue la visibilité des dirigeants et responsables.


Dans le cadre de sa stratégie destinée à intégrer les nouvelles technologies dans ses métiers et ses prestations, et avec l'augmentation des activités de l'entreprise, une solution numérique pour gérer ses propres projets et ses affaires commerciales est devenue obligatoire.
...
\begin{enumerate}[label=(\alph*)]
\item {\textbf{\textbf{Documatation et formation}}}
	 \paragraph{}
	...
	\paragraph{}
 Nous avons suivi une   formation sur la plateforme de Salesforce nommée Trailhead : Trailhead permet d'accélérer la carrière en permettant d'acquérir ou d'approfondir les compétences Salesforce. \paragraph{}
Quel que soit le rôle (développeur, utilisateur, administrateur) ou le niveau (débutant ou expérimenté), Trailhead aide à comprendre grâce à ses différents parcours et modules, la plateforme applicative (App Cloud), le CRM (Sales Cloud) ou encore la plateforme analytique (Analytics Cloud). Les connaissances acquises sont certifiées au moyen de badges tout au long de l'apprentissage. 
\paragraph{}Cet apprentissage se veut par ailleurs très pratique et concret puisque les badges s'obtiennent après une mise en pratique des connaissances acquises via de nombreux challenges (quizz, exercices...). Enfin, des projets concrets permettent d'appliquer son savoir dans des mises en situation aussi réalistes que possible (comme développer une application de gestion de conférences). Depuis son lancement, plus de 250 000 badges ont été attribués sur Trailhead. 
\paragraph{}
...
\begin{figure}[H]
\centering
\includegraphics[scale=0.45]{rank.png}
\caption{Interface de la plateforme de formation TrailHead .}
\end{figure}
\item{\textbf{Conception} }
\paragraph{}De tous les  mots  associés  au  e-business, aucun n'est autant exploité que "processus". Prenez  
l'expression "processus   business"   et   ajouter   l'un ou l'autre des qualificatifs "gestion","intégration", "optimisation", "automatisation", "modélisation" ou "simulation". Au final, ces combinaisons forment des notions qui ont un sens dans un contexte  historique mais qui sont totalement interchangeables dans le discours des vendeurs de software, car ils espèrent positionner leurs produits avec le maximum de visibilité dans la sphère très à la mode des outils liés aux processus business. Ces derniers temps, une de ces expressions se détache nettement des autres  et  il  s'agit  de  "gestion  des  processus  business"  ou  BPM  (Business  Process  Management).  
\paragraph{}
Un système informatique de BPM\footnotemark[1] permet la conception, l'analyse,  l'optimisation et l'automatisation  des  processus  business.  Pour y parvenir,  il  sépare  la  logique  du  processus  des  
applications qui soutiennent  le processus,  il  gère  les relations  entre  les  différents  intervenants,  il intègre les  ressources  internes  et  externes  de  l'entreprise  nécessaires  pour  le  déroulement  du processus  et  enfin  il  surveille  le  déroulement  du  processus  pour  en  tirer  une  analyse  des  
performances qui serviront à le faire évoluer. 
\footnotetext[1]{Business Process Modeler : est un  logiciel qui permet d'identifier les différents processus et les rationaliser.}
\paragraph{}
Ce travail de mémoire tente de faire une synthèse des différents aspects qui gravitent autours de la gestion des processus business, des concepts de bases aux acteurs du marché en passant par les différentes fonctionnalités d'un système de BPM et l'implication sur les structures organisationnelles de l'entreprise.
\paragraph{}
 Cette partie de la conception a consisté à définir les processus businesspuis à les modéliser de manière conceptuelle par 
la méthode OSSAD au niveau abstrait (sous la forme de fonctions, sous-fonctions, activités et paquets). Dans un deuxième temps, le prototype d'une  application  a  été  développé.  Cet  outil  permet  de  tester  l'intégrité  de  la  modélisation  et  
facilite la consultation de cette modélisation soit en local ou sur le cloud, car dans notre nous avons utiliser l'outil "Bizagi" 
\pargraph{}Dans un autre temps Nous avons fourni le data model adéquat pour l'application qui représente les différents objets standards et personnalisés à utiliser lors du développement de l'application. Ainsi l'élaboration de différents diagrammes selon les acteurs du système. 
...
\paragraph{}
Parmi les outils de planification de projet, on a utilisé le diagramme de GANTT,c'est un outil qui permet de planifier le projet, ordonnancer les tâches et estimer leurs charges respectives.
\paragraph{}
Ce diagramme permet aussi de visualiser l'enchainement et la durée des différentes tâches durant la réalisation du projet. (voir Annexe)

\item{\textbf{Réalisation}} la réalisation s'est déroulée sur plusieurs étapes, selon les processus détérminés pendant la phase de la conception initiale.
\paragraph{}
chaque fois qu'on est concentré sur un processus, on refait la conception avec plus de détails sur les champs à créer, les règles de validation et les workflow, les reunions se déroulaient au sujet du processus en question uniquement.
\paragraph{}
On commence à developper les fonctionnalitées demandées avec les tests en parallèle, une fois les objectifs de l'iteration atteints, on fait un démo des fonctionnalités developpées.
\paragraph{}
Après la démo soit on passe à la partie suivante, sinon on ajoute quelques jours pour la finalisation des fonctionnalités déjà developpées.`
\end {enumerate}
\subsection{Conduite du projet}
Le respect des délais est l'un des enjeux majeurs du management de projet. Ainsi, la planification de projet est un outil incontournable qui permet de définir les travaux à réaliser, fixer les objectifs, coordonner les actions, maîtriser les moyens, diminuer les risques, suivre les actions en cours et rendre compte de l'état d'avancement du projet.
Nous avons donc jugé nécessaire de préparer un planning prévisionnel qui donne un aperçu général sur notre projet de fin d’études, et aussi de suivre une approche méthodologique agile qui peut répondre au changement continue.
\begin{enumerate}[label=(\alph*)]
\item{\textbf{Pratiques SCRUM}}
\paragraph{}La gestion de projet ou conduite de projet est une démarche visant à structurer, assurer et optimiser le bon déroulement d'un projet suffisamment complexe.
\paragraph{}
Ainsi Pour optimiser le temps disponible, nous avons travaillé en mode agile, c'est à dire en cycles itératifs courts permettant de fournir rapidement les fonctionnalités essentielles qui rendront l'application utilisable est finctionnelle.
\paragraph{}
...
Les fonctionnalités sont ensuite implémentées et durant toute cette phase, le « contenu » de l'itération est fixe, mais des modifications quotidiennes légères peuvent avoir lieu.
\item Test Unitaire:
les classes de teste sont écrite en parallèle avec le développement de la classe , IDE de Salesforce calcule le niveau de recouvrement des tests.
...
\section{Gestion des Relations Clients CRM }
\paragraph{}
Avant d'entamer la partie pratique de notre projet, il nous est paru judicieux de définir, dans un premier temps, la notion du CRM ou la Gestion des Relations Clients comme un ensemble de solutions technologiques spécialisées, permettant aux entreprises d'assurer un suivi de leur clientèle et de personnaliser leurs communications avec leurs marchés.
\subsection {Définition}
 Le CRM est un processus mettant en œuvre outils logiciels, méthodes, stratégie et comportements pour gérer plus efficacement la relation avec le client.
\paragraph{} Le processus commence dès l'étape de prospection des nouveaux clients et se poursuit en une recherche continue de fidélisation de clients à fort potentiel. D'une autre approche : C'est une méthode globale pour optimiser ses relations avec ses contacts (clients, prospects, prescripteurs...) afin d'avoir une meilleure connaissance de leurs besoins et d'y répondre le mieux possible ; tout en optimisant la rentabilité de cette relation. 
\paragraph{}Il vise à proposer des solutions technologiques permettant de renforcer la communication entre l'entreprise et ses clients afin d'améliorer la relation avec la clientèle en automatisant les différentes composantes de la relation client :  
\begin{enumerate}[label=(\alph*)]
	\item 
...
\paragraph{} La notion d'évolutivité et d'élasticité s'explique par le fait que l'utilisateur peut faire varier les ressources demandées à la
...
de fermes de serveurs géantes (également appelées Datacenters). \paragraph{}Ceci est rendu possible par le procédé de virtualisation qui consiste à faire fonctionner plusieurs systèmes d'exploitation ainsi que leurs applications associées sur un seul serveurphysique. La virtualisation permet ainsi de recréer plusieurs ordinateurs virtuels sur une seule et même machine physique.
...
\section {La philosophie de Salesforce}

toute entreprise a  besoin  d'avoir un aperçu global sur les données de la société, et cela commence par les relations commerciales. Dans Salesforce, on stocke des informations à prepos de clients en utilisant des comptes et des contacts. 

\begin{enumerate}[label=(\alph*)]
\item {\textbf{Compte : }} Les comptes sont les sociétés avec lesquelles nous somme en affaire.
\item{\textbf{Contact : }}
 les contacts sont des personnes qui travaillent pour ces sociétés.
\item{\textbf{Opportunités: }}
Les opportunités sont des affaires en cours. Dans Salesforce, vous pouvez créer des opportunités pour des comptes existants ou en convertissant une piste qualifiée.
\paragraph{}

Dans le secteur des ventes, nous savons que les affaires passent de « provisoires » à « fermes » avant d'être finalisées. Au fur et à mesure que l'affaire évolue, nous devons de plus en plus confiant quant à la conclusion de la vente. Dans Salesforce, une opportunité passe par une série d'étapes liée au type de tâches exécutées, à qui a l'initiative et à la probabilité de conclure la vente.

Voici certaines des étapes par lesquelles on passe  habituellement :

    
		\begin{itemize}
			\item		Prospection
    	\item Développement
    	\item Négociation/Vérification
    	\item Clôturée/Gagnée
    	\item Clôturée/Perdue
		\end{itemize}

\item{\textbf{Piste :} }piste dans salesforce.com sont ceux qui nous somme intéressé à travailler avec, mais on ne sait pas s'ils sont intéressés à travailler avec nous.
\paragraph{}
On peut avoir les piste à partir d'une liste que nous avons acheté, ou d'un ensemble de cartes d'affaires que nous avons recueillies lors d'un salon commercial ou autres. Dans tous les cas, nous devons communiquer avec le responsable pour savoir s'ils sont intéressés à notre produit ou service. \paragraph{}
Salesforce.com est un gestionnaire ou un système de suivi des piste, qui utilise un processus de qualification des prospects pour savoir si la piste est qualifié (pourrait être le client), ou non (ne sera pas un client).

Un exemple de processus de qualification des pistes est:
\begin{itemize}
\item ouverte :Non contactée (l'étape par défaut de tous les pistes)
\item travaillé - Contactée
\item Fermée -- convertie 
\item Fermée/pas convertie
\end{itemize}
ces informations sont suivies à partir de du champs Status de l'Objet Piste.
\paragraph{}
Si une piste est "Fermée" - Non convertie, c'est à dire que il n y a la possibilité de faire des affaires avec cette piste pendant une période de temps.
\paragrpah{} Si la piste  est "Fermée" -Convertie alors que nous avons remarqué qu'il y a un certain intérêt de leur part à faire des affaires avec nous, ainsi nous voulons commencer notre processus de vente. et on converti.
 \paragraph{}
Une piste convertie cela signifie que cette piste est transformée à un contact (personne), compte (entreprise), et opportunité(possibilité de vente) dans le système Salesforce.com.
\paragraph{}
Il est possible qu'une piste devient un contact et un compte sans devenir une opportunité, si on veut qu'elle devient partie de notre réseau, mais nous n'avons pas l'intention de vendre pour eux en ce moment. 
\paragraph{}
Salesforce agit comme un système de gestion des contacts et un système de suivi de piste, jusqu'à ce qu'un contact ou une piste est convertie en une opportunité, Salesforce devient une solution pour la gestion des ventes
\paragraph{}
Le processus d'automatisation des ventes dans Salesforce définit la probabilité de conclure une vente, qui est ensuite utilisé pour la prévision des ventes. Par exemple, si l'étape de vente "Décideur - contacté" a une probabilité de 40  \% pour fermer et je suis en train de travailler sur une opportunité de 100 000 \$, puis ma prévision de ventes dans le tableau de bord Salesforce indique que l'opportunité va générer 40 000 \$ en revenus.
\end{enumerate}
%-_-_-_-_-__-_-_-_ Fin CRM, Cloud Computing-_-_-_-_-_-

%-_-_-_-_-_-_-_-_-_ Analyse Fonctionnelle et technique_-_-_-_-
\chapter{Analyse Fonctionnelle et Technique}
%---resumé 
\begin{tcolorbox}

Ce chapitre présente une étude détaillée des spécifications fonctionnelles du projet.          Où traite les besoins fonctionnels et non fonctionnels du projet visant à reformuler le besoin d’une manière simple et réalisable.
La deuxième section de ce chapitre traite l’étude technique au long de laquelle sont exposés, dans un premier temps, la solution Salesforce CRM en décrivant l’architecture, les applications et les fonctionnalités de la plateforme et dans un deuxième temps les technologies de travail.
...
\begin{enumerate}[label=(\alph*)]
	\item {	\textbf{Processus de Marketing}}
...
\begin{figure}[H]
\centering
\caption{Modèle du Processus de l'email marekting}
\includegraphics[scale=0.75]{processusmarketing.png}
\end {figure}
\item{\textbf{Processus de ventes} }
Le processus de vente commence par la création d'une piste, par la suite cette piste est soit supprimée si elle n'est pas qualifiée, soit qualifiée et convertie.\paragraph{}
...
\item {\textbf{Le processus de facturation} }
...
\end{figure}
\paragraph{}
\textcolor[rgb]{1,1,1}{tfghjjkkn}
\newline
\newline
\newline
\item {\textbf{Lightning Sync : }}
Google a maintenant des fonctions de calendrier. La synchronisation du calendrier de salesforce avec le calendrier Google nous permettrait de fournir des tâches à un ensemble diversifié d'outils utilisés dans le domaine.  Cela permettrait aux clients de numériser des calendriers de vente pour réserver l'heure et nous pourrions lier cela à Salesforce. Le calendrier Google peut être mis à jour depuis le téléphone portable ( depuis l'Agenda de Google). 
cette figure montre le résultat final de cette configuration dans Google Contacts:
\begin{figure}[H]
	\centering
		\includegraphics{Contactsync.PNG}
	\caption{ capture: Contacts Synchronisés de SF vers Google Contacts}
\end{figure}
 \paragraph{}
A la fin de cette configuration de Google Sync j'ai rédigé un Guide de configuration qui est publier sur le blog de Neoxia, sous le lien suivant:
http://blog.neoxia.com/guide-to-configure-lightning-sync-for-googlebeta-2/
...
\paragraph{SFDC MVC }
on peux écrire les pages "`VUE" en utilisant SFDC\foornotemark[1] visualForce (VF). les pages VF sont similaires aux pages JSP\foornotemark[2].chaque page VF est associée à un Controlleur.soit on utilise les Controlleurs Standard ou écrire nos propre controlleurs en utlisant le language Apex. 
\foornotetext[1]{}
\footnotetext[2]{}
\paragraph{}
...
\end{enumerate}
\paragraph{Modèle}
le modèle et le schéma et les données qu'utilise salesforce pour représenter le système complèt. Dans SF\foornotemark[4] on peut dire que \textbf{sObjects}sont le modèle, comme toute entité dans SF est mappée avec un sObject.
\paragraph{Vue}
...
\paragraph{}
SALESFORCE est une PAAS\footnotemark[1] hautement personnalisée. En plus de la multitude des fonctionnalités offertes pour les utilisateurs de la plate-forme, cette dernière permet d'étendre ses fonctionnalités standard, créer des pages, des composants et des applications entièrement personnalisées.
...
%---- tesumé 
Ce dernier chapitre est consacré à la réalisation du projet, il est composé en plusieurs sections, chaque section correspond à un processus métier, où on va analyser et traiter les spécifications et les fonctionnalités des processus,
Pour chaque partie, on élabore une conception précise et une modélisation complète, suivi d’une explication des étapes de la réalisation ainsi que les difficultés rencontrées lors du développement.
...
\begin{figure}[H]
	...
	\caption{le schéma des objets du processus de vente}
\end{figure}
...
\begin{figure}[H]
	...
	\caption{le schéma des objets du processus de marketing}
\end{figure}
...
\begin{enumerate}
	...
	\begin{figure}[H]
		...
		\caption{Interface de configuration de "Email to SalesForce"}
	\end{figure}
	...
	\begin{figure}[H]
		...
		\caption{stratégie de configuration de linghning Sync}
	\end{figure}
...
\begin{figure}[H]
	\centering
		\includegraphics[scale=0.6]{googlescript.PNG}
...
\begin{figure}[H]
	\centering
		\includegraphics[scale=0.6]{reponsemail.PNG}
...
\subsection{Réalisation et développement}
La première étape dans cette phase configurer les objets personnalisés; Facture, Echeance et poste d'echeance, c'est à dire créer tous les champs necessaire pour chaque objets avec les règles de validation convenables et les worklfow qui mène le bon déroulement du processus; 
\paragraph{}
l'enjeu de ce processus et la miltitudes des types de projet, sachant que le processus de facturation dépend du type de projet:
\begin{itemize}
	\item  Projet forfait : Comme son nom l'indique, le développement au forfait se calcule sur le projet et non sur le temps passé réellement. Le coût peut donc être parfois plus élevé qu'avec une facturation en régie mais il est ferme et définitif. Comme son nom l'indique, le développement au forfait se calcule sur le projet et non sur le temps passé réellement. Le coût peut donc être parfois plus élevé qu'avec une facturation en régie mais il est ferme et définitif. 
	\item Projet TMA : La TMA définit seulement la maintenance d'une ou plusieurs applications, sans nécessiter d'évolutions majeures sur l'une d'entre elles. Cette option peut également se faire en régie dans le cas de gros projets.La TMA est là pour répondre à des besoins importants de grandes sociétés, dans ce cas des équipes entières peuvent être envoyées en régie chez le client pour assurer la maintenance d'un lot d'application.
	\item Projet en régie : La facturation en régie consiste à faire payer le temps réellement passé pour le service. Cela exige de bien estimer au départ la durée du service à effectuer. Malgré toutes les précautions prises au départ, un imprévu peut toujours se produire et bouleverser la donne.le développement en régie est privilégié pour les missions à durée bien définie. 
\end{itemize}



\section{Etablissement des rapports et des tableaux de bord}
...
\begin{figure}[H]
	\centering
		\includegraphics[scale=0.45]{rapportettb.PNG}
...
\begin{figure}[H]
	\centering
		\includegraphics[scale=0.7]{dashbord.PNG}
...
\begin{figure}[H]
	\centering
		\includegraphics[scale=0.6]{rapports.PNG}
	\label{fig:rapports}
\end{figure}
 
\section{La stratégie de test}
\paragraph{}
les tests sont la clé de succès du développement à long terme et est un élément essentiel du processus de développement. c'est pourquoi Nous vous utiliser un processus de développement piloté par les tests, c'est l'élaboration d'un test, qui se produit en même temps que l'élaboration du code.
\paragraph{}
les tests unitaire avèrent très importants en particulier si l'application est à déployer sur les clients. Si on peut confirmer que l'application fonctionne comme prévu, qu'il n'y a pas de comportements inattendus, les clients vont nous faire confiance plus.
\paragraph{}
Il y a deux façons de tester une application. L'un est à partir de l'interface utilisateur Salesforce, mais simplement un test à l'interface utilisateur ne pourra pas capturer tous les cas d'utilisation de l'application. L'autre façon est de tester les fonctionnalités en bulk : jusqu'à 200 enregistrements peut être passé à travers le code s'il n'est invoquée à l'aide de l'API SOAP ou par un contrôleur standard de Visualforce.
\paragraph{}
Une application est rarement terminée, on aura toujours de nouvelles versions pour la même application, où on change et étend les fonctionnalités. Si nous avons écrit des tests complets, nous pouvons assurer qu'on aura pas de régression on ajoutant de nouvelles fonctionnalité.
d'ailleurs dans Salesforce pour déployer le code ou de l'emballer dans un package pour le AppExchange de Force.com?,  les conditions suivantes doivent être remplies:
\begin{itemize}
	\item 
Au moins 75  \% du Code  Apex doit être couvert par des tests unitaires, et l'ensemble de ces tests doivent compléter avec succès.
\item Chaque déclenchement doit avoir une certaine couverture de test.
\item Toutes les classes et les déclencheurs doivent compiler avec succès.
\end{itemize}

\paragraph{}
Lors du déploiement de l'Apex dans l'organisation de production, chaque test unitaire dans l'organisation est exécuté par défaut.
\subsubsection{Exemple de code}
celà est un exemple d'une classe de test qui contient deux méthodes
\begin{figure}[H]
	\centering
...
\paragraph{}
Ce projet de fin d'études, effectué au sein de Neoxia MAROC qui a durée une période de 5 mois, consiste à la réalisation d'une Solution CRM pour la gestion du processus commercial de Neoxia Maroc à base du CRM Salesforce.
\paragraph{}
L'augmentation du volume de l'entreprise dans le marché, veut dire un plus grand nombre de projet et d'affaires, cela rend le suivi de l'activité de l'entreprise difficile, qui dis moins de visibilité dis difficulté de gestion pour cela Neoxia a investi pour créer une Solution CRM.
Le CRM permet de regrouper toute l'information d'un contact, qu'il soit client, prospect ou même fournisseur, à l'intérieur d'un seul et même endroit en particulier l'application réalisée permettra d'automatiser et de renforcer les forces de ventes, ainsi le suivi des projets et des imputations.

\paragraph{}
Le CRM Salesforce et un outil presque complet et performent, mais il y a toujours des limitations surtout que le métier informatique exige beaucoup de spécifications, pour cela nous avons développé plusieurs processus bien adaptés au métier de l'entreprise, comme nous avons contenté de personnaliser et configurer les processus standard de Salesforce.
...
\paragraph{}
La première itération du projet a consisté en la définition du périmètre du projet et l'étude de l'existant. Les itérations qui suivent ont été consacrées à la documenttion, la formation et  la réalisation de plusieurs modules métier qui ont une grande valeur ajoutée pour les utilisateurs de la plateforme. Lors de chaque itération, une étude fonctionnelle a été effectuée suivie d'une étude technique si nécessaire. Enfin une conception détaillée est mise en place, pour pouvoir par la suite enchainer sur la phase de réalisation et de développement des modules en question.
\paragraph{}
Comme le présent rapport a été rédiger avant la fin de la période de stage, une deuxième parties est n'est pas encore commencée; 
dans le reste de la période du stage on va travailler sur tous en  tous ce qui concerne les projets et les imputations, on va introduire la notion du projet dans salesforce, le suivi des projet, les technologies utilisés ainsi que la gestion des imputations, celà en synchronisant la plateforme Salesforce avec l'outil redMine.
...
\chapter*{\textsc{Annexe}}


\addcontentsline{toc}{chapter}{\textsc{Annexe}}
