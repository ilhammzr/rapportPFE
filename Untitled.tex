...
\item \textbf{Le dernier chapitre} est consacré à la mise en œuvre du projet en décrivant toutes les étapes par lesquelles nous sommes passés pour donner naissance à notre application depuis la configuration des objets personnalisés à la génération des rapports et tableaux de bord.
\end{itemize}
...
\paragraph{}
Neoxia comme un cabinet de conseil qui vient d'être installé sur 
le terrain marocain, cherche à gangner les bons marché ainsi de garder et fédiliserles bons clients.
 \paragraph{}
Durant ses 4 ans d'expérience, Neoxia remarque que son volume dans le marché marocain augmente au niveau du nombre des clients, les marchés gagnés, les actions commerciales et marketing, le nombres des technologies utilisée ainsi que la multitude des profils des Neoxiens, avec cette augmentation, le suivi des activité de l'entreprise devient difficile, à voir  le traitement de tous ce qui concerne l'identification des clients et surtout les besoins et attentes fluctuants.
...
les objets :
Piste (Lead):piste dans salesforce.com sont ceux qui nous somme intéressé à travailler avec, mais on ne sais pas s'ils sont intéressés à travailler avec nous.
\paragraph{}
On peut avoir les piste à partir d'une liste que nous avons acheté, ou d'un ensemble de cartes d'affaires que nous avons recueillies lors d'un salon commercial. Dans les deux cas, nous devons communiquer avec le responsable pour savoir s'ils sont intéressés à notre produit ou service. \paragraph{}
Salesforce.com est un gestionnaire et un système de suivi des piste, qui utilise un processus de qualification des prospects pour savoir si la piste est qualifié (pourrait être le client), ou non (ne sera pas un client).

Un exemple de processus de qualification des pistes est:
\begin{itemize}
\item ouverte :Non contactée (l'étape par défaut de tous les pistes)
\item travail - Contactée
\item Fermée -- convertie 
\item Fermée/pas convertie
\end{itemize}
ces informations sont suivies à partir de l'objet Status de l'Objet Piste.
\paragraph{}
...
\paragrpah{} Si la piste  est "Fermée" -Convertie alors nous avons remarqué qu'il y a un certain intérêt de leur part à faire des affaires avec nous, ainsi nous voulons commencer notre processus de vente. et on converti.
 \paragraph{}
...
\paragraph{}
Il est possible qu'un piste devient un contact et un compte sans devenir une opportunité, si on veut qu'ils deviennent partie de notre réseau, mais nous n'avons pas l'intention de vendre pour eux. 
\paragraph{}
...
Le processus d'automatisation des ventes dans Salesforce définit la probabilité de conclure une vente, qui est ensuite utilisé pour la prévision des ventes. Par exemple, si l'étape de vente "Décideur - contacté" a une probabilité de 40  \% pour fermer et je suis en train de travailler sur une possibilité de 100 000 \$, puis ma prévision de ventes dans le tableau de bord Salesforce indique l'occasion va générer 40 000 \$ en revenus. Comme je obtenir plus loin sur l'étape de vente la probabilité augmente et le revenu prévu va augmenter.

Nous recommandons qu'une entreprise ont moins d'étapes de vente, plutôt que de nombreuses étapes de vente, et que chaque étape de vente être associé à une action.

Les ventes sont assez dur sans avoir à se souvenir d'une scène 15 processus de vente. Concentrer votre énergie sur la conclusion de la vente non pas sur la réflexion sur ce que vous êtes en phase de vente. Rendre le processus d'automatisation des ventes moins complexe, mais en correspondance avec votre processus de vente, augmente l'adhésion des utilisateurs.

Lorsque vous fermez la vente et obtenir l'analyse de l'étape de vente devient "Fermé - Gagné." Si vous voulez vous concentrer sur la droite mène, contacts et opportunités menant à de plus en plus de victoires, puis à l'aide d'un système d'automatisation de la force de vente et la création d'un processus d'automatisation de la force de vente qui reflète votre entreprise est essentielle.

Le concept clé lors de la création de votre processus d'automatisation des ventes, est de passer par elle avec de vrais exemples de clients pour voir si vos ventes étapes permettent suffisamment de souplesse et que le processus de vente CRM de Salesforce est utile pour toutes les personnes impliquées dans le processus de vente.
